...
```bash
# Clone
git clone [https://github.com/](https://github.com/)[본인아이디]/jcloud-bookstore.git
cd jcloud-bookstore
...
🔐 인증 및 권한 (Auth & Roles)
인증 플로우
POST /api/v1/auth/login으로 이메일/비번 전송.

응답받은 access_token을 HTTP Header에 포함하여 요청.
...
동기식 DB 처리: 트래픽 증가 시 성능 저하 우려. 추후 async/await를 완벽 지원하는 비동기 드라이버(aiomysql)로 교체 고려.
